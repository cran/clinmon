...
PI <- function(
   ...
   df_agg <- z_agg(df,freq,blocksize,blockmin,by_type=c("max","min","mean"),n_vars=1)

   results <- cm_pi_output(df, df_agg, by_type=c("val1_max","val1_min", "val1_mean"),freq, blocksize, output)
   ...
   colnames(results)[colnames(results) == "val1_mean"] <- paste0(df_cols[2],"_mean")
